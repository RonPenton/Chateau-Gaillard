...
80 IF ROOMS(RO,7)<>0 OR ROOMS(RO,9)<>0 OR ROOMS(RO,1O)<>0 THEN GOSUB 6660:REM OBJECTS
90 IF ROOMS(RO,7)>98 THEN PRINT "One of the doors is locked":PRINT "preventing you from exploring":PRINT "further"
100 IF ROOMS(RO,8)<>0 THEN PRINT TAB(3);"LOOK OUT!":PRINT "THERE IS A ";MONSTERNAMES(ROOMS(RO,8));" HERE!":IF RND(1)>.7 AND ROOMS(RO,8)<>1 THEN PRINT "THE ";MONSTERNAMES(ROOMS(RO,8));" ATTACKS!":KW=1:GOSUB 1400:GOTO 40
110 IF ST<1 THEN ST=0 ELSE IF RND(1)>.84 THEN ST=ST-1
...
170 IF RND(1)>.84 AND RO=16 AND ROOMS(RO,8)=1 THEN PRINT:PRINT "You hear a whispered voice warning you:":PRINT "'You must do something about the dwarf'"
...
230 FLAG=0
240 FOR J=1 TO 5
250 IF INVENTORY(J)<>0 THEN FLAG=1
260 NEXT J
270 IF FLAG=0 THEN 340
280 CASH=0
290 PRINT:PRINT "You are carrying:"
300 FOR J=1 TO 5
310 IF INVENTORY(J)<>0 THEN PRINT TAB(4);ITEMNAME(INVENTORY(J)): CASH=CASH+ITEMVALUE(INVENTORY(J))
320 NEXT J
...
550 IF ROOMS(RO,7)>98 AND B$<>"UNL" THEN PRINT TAB(4);"** The doors are locked **":GOTO 40
...
730 REM ********
740 REM MOVEMENT
750 IF RO<>16 OR ROOMS(16,8)<>1 THEN 800
...
790 RETURN
800 C$=LEFT$(C$,1)
810 IF C$="N" AND ROOMS(RO,1)=0 THEN PRINT
"You cannot go that way":RETURN
820 IF C$="S" AND ROOMS(RO,2)=0 THEN PRINT
"There is no exit south":RETURN
830 IF C$="E" AND ROOMS(RO,3)=0 THEN PRINT
"I see nowhere to the east to go":RETURN
840 IF C$="W" AND ROOMS(RO,4)=0 THEN PRINT
"Even you cannot walk through
walls":RETURN
850 IF C$="U" AND ROOMS(RO,5)=0 THEN PRINT
"There is no way to move up":RETURN
860 IF C$="D" AND ROOMS(RO,6)=0 THEN PRINT
"You cannot descend from here":RETURN
870 IF C$="N" THEN RO=ROOMS(RO,1)
880 IF C$="S" THEN RO=ROOMS(RO,2)
890 IF C$="E" THEN RO=ROOMS(RO,3)
900 IF C$="W" THEN RO=ROOMS(RO,4)
910 IF C$="U" THEN RO=ROOMS(RO,5)
920 IF C$="D" THEN RO=ROOMS(RO,6)
930 RETURN
940 REM *******************
950 REM GET OBJECTS ROUTINE
960 FLAG=0
970 FOR J=1 TO 5
980 IF INVENTORY(J)<>0 THEN FLAG=FLAG+1
990 NEXT J
...
1020 IF ROOMS(RO,7)=0 AND ROOMS(RO,9)=0 AND ROOMS(RO,10)=0
THEN PRINT "I see nothing to pick up"
:RETURN
1030
D$="":E$="":F$="":N=0
1040 D$=LEFT$(ITEMNAME(ROOMS(RO,7)),3)
1050 E$=LEFT$(ITEMNAME(ROOMS(RO,9)),3)
1060 F$=LEFT$(ITEMNAME(ROOMS(RO,10)),3)
1070 IF C$=D$ THEN N=ROOMS(RO,7):ROOMS(RO,7)=0
1080 IF C$=E$ THEN N=ROOMS(RO,9):ROOMS(RO,9)=0
1090 IF C$=F$ THEN N=ROOMS(RO,10):ROOMS(RO,10)=0
1100 J=1
1110 IF INVENTORY(J)=0 THEN INVENTORY(J)=N:GOTO 1130
...
1170 FLAG=0
1180 FOR J=1 TO 5
1190 IF INVENTORY(J)<>0 THEN FLAG=1
1200 NEXT J
1210 IF FLAG=0 THEN PRINT "You are not
carrying anything":RETURN
1220 IF ROOMS(RO,7)<>0 AND ROOMS(RO,9)<>0 AND
ROOMS(RO,10)<>0 THEN PRINT "This room
already holds its":PRINT TAB(6);"maximum
objects":RETURN
1230 D$="":D=0
1240 FOR J=1 TO 18
1250 IF LEFT$(ITEMNAME(J),3)=C$ THEN D$=ITEMNAME(J):D=J
1260 NEXT J
1270 IF D$="" THEN PRINT
TAB(3);"How can you when you're":PRINT
TAB(5);"not holding it?":RETURN
1280 FOR J=1 TO 5
1290 IF INVENTORY(J)=D THEN INVENTORY(J)=0
1300 NEXT J
1310 IF ROOMS(RO,7)=0 THEN ROOMS(RO,7)=D:GOTO 1340
1320 IF ROOMS(RO,9)=0 THEN ROOMS(RO,9)=D:GOTO 1340
1330 IF ROOMS(RO,10)=0 THEN ROOMS(RO,10)=D
...
empty air!"
1460 RETURN
1470 G$=MONSTERNAMES(ROOMS(RO,8))
1480 S1=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
1490 H1=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
1500 D1=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
...
wounded!!" :HT=HT-floor(RND(1)*3)-1:DE=DE-1
...
TIME..."
2270 IF Z=1 OR Q=1 THEN ST=4*floor(ST/5)
2280 IF Z=2 OR Q=2 THEN CH=3*floor(CH/4)
2290 IF Z=3 OR Q=3 THEN DE=6*floor(DE/7)
2300 IF Z=4 OR Q=4 THEN IT=2*floor(IT/3)
2310 IF Z=5 OR Q=5 THEN WI=5*floor(WI/6)
2320 IF Z=6 OR Q=6 THEN CO=floor(CO/2)
2330 ROOMS(RO,8)=0
2340 RETURN
2350 REM ************
2360 REM UNLOCK DOORS
2370 IF RO<>8 AND RO<>34 OR
ROOMS(RO,7)<99 THEN PRINT "There is no locked
door in this room":RETURN
2380 FLAG=0
2390 T=0
2400 FOR J=1 TO 5
2410 IF INVENTORY(J)=17 THEN FLAG=1:T=J
2420 IF INVENTORY(J)=18 THEN FLAG=2:T=J
2430 NEXT J
...
2880 FLAG=0
2890 FOR J=1 TO 5
2900 IF INVENTORY(J)=13 THEN FLAG=J
2910 NEXT J
...
view..."
2960 ST=20
2970 INVENTORY(FLAG)=0
2980 RETURN
2990 REM ***********
3000 REM BRIBE DWARF
3010 IF ROOMS(RO,8)<>1 THEN PRINT "YOU
SHOULDN'T TRY THAT":PRINT "WITH A
";MONSTERNAMES(ROOMS(RO,8)):RETURN
3020 PRINT "He demands the amulet!"
3030 FLAG=0
3040 FOR J=1 TO 5
3050 IF INVENTORY(J)=9 THEN FLAG=J
3060 NEXT J
...
3130 FOR J=1 TO 5
3140 IF INVENTORY(J)<>0 THEN FLAG=J
3150 NEXT J
3160 IF FLAG=0 THEN 3200
3170 PRINT "THE ";ITEMNAME(FLAG)
3180 INVENTORY(FLAG)=0
3190 GOTO 3270
...
6670 PRINT
6680 IF ROOMS(RO,7)>98 AND ROOMS(RO,9)=0 AND ROOMS(RO,10)=0
THEN RETURN
6690 PRINT TAB(3)"YOU CAN SEE ";
6700 IF ROOMS(RO,7)>98 THEN 6720
6710 IF ROOMS(RO,7)<>0 THEN PRINT ITEMNAME(ROOMS(RO,7))
6720 IF ROOMS(RO,9)>98 THEN 6740
6730 IF ROOMS(RO,9)<>0 THEN PRINT ITEMNAME(ROOMS(RO,9))
6740 IF ROOMS(RO,10)>98 THEN 6760
6750 IF ROOMS(RO,10)<>0 THEN PRINT ITEMNAME(ROOMS(RO,10))
...
6900 DIM ROOMS(44,10),INVENTORY(5),ITEMNAME(20),ITEMVALUE(20),MONSTERNAMES(20),T(5)
6910 ST=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
6920 CH=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
6930 DE=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
6940 IT=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
6950 WI=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
6960 CO=floor(RND(1)*6+RND(1)*6+RND(1)*6)+3
...
7020 ROOMS(8,7)=99:REM LOCKED DOOR i
7030 ROOMS(34,7)=100:REM LOCKED DOOR ii
7040 ROOMS(40,7)=20:REM CHEST
7050 ROOMS(13,7)=19:REM CHEST
7060 ROOMS(16,8)=1:REM DWARF
...
7140 RESTORE 7830
7150 FOR Z=1 TO 20
7160 READ ITEMNAME(Z), ITEMVALUE(Z)
7170 NEXT Z
7180 FOR Q=1 TO 16
7190 Z=floor(RND(1)*44)+1
7200 IF Z=5 OR Z=17 OR Z=27 OR Z=29 OR Z=30 OR
Z=31 OR Z=32 OR Z=37 THEN 7190
7210 IF ROOMS(Z,7)<>0 THEN 7190
7220 ROOMS(Z,7)=Q
7230 NEXT Q
7240 PRINT
